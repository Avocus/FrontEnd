...
const SidebarItem = ({ label, href, icon, active, onClick, collapsed }: SidebarItemProps) => {
...
export function AdvogadoSidebar({ 
  ...
  const casosItems: SidebarItemProps[] = [
    ...
    { label: "Documentos", href: "/documentos", icon: <FileText className="h-5 w-5" /> },
    { label: "Clientes", href: "/clientes", icon: <Users className="h-5 w-5" /> },
  ];

  const gerencialItems: SidebarItemProps[] = [
    { label: "Estatísticas", href: "/estatisticas", icon: <BarChart className="h-5 w-5" /> },
    { label: "Prazos", href: "/prazos", icon: <Clock className="h-5 w-5" /> },
